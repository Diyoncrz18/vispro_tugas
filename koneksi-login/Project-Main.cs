...
namespace koneksi_login
{
    ...
    public partial class Project_Main : Form
    {
        ...
        public Project_Main()
        {
            alamat = "server=localhost; database=db_dion; username=root; password=;";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtUsername.Text != "")
                {
                    query = string.Format("select * from db_dion where username = '{0}'", txtUsername.Text);
                    ds.Clear();
                    koneksi.Open();
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {
                            txtID.Text = kolom["usr_id"].ToString();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtPassword.Text != "" && txtNama.Text != "" && txtUsername.Text != "" && txtID.Text != "")
                {
                    ...
                    query = string.Format("UPDATE db_dion SET password = '{0}', namapengguna = '{1}', level = '{2}' WHERE usr_id = '{3}'",
        ...
        private void Project_Main_Load(object sender, EventArgs e)
        {
            ...
            {
                koneksi.Open();
                query = string.Format("select * from db_dion");
                perintah = new MySqlCommand(query, koneksi);
                adapter = new MySqlDataAdapter(perintah);
                perintah.ExecuteNonQuery();
                ds.Clear();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtUsername.Text != "" && txtPassword.Text != "" && txtNama.Text != "")
                {
                    ...
                    query = string.Format("insert into db_dion  values ('{0}','{1}','{2}','{3}','{4}');", txtID.Text, txtUsername.Text, txtPassword.Text, txtNama.Text, CBLevel.Text);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtID.Text != "")
                {
                    ...
                    {
                        query = string.Format("Delete from db_dion where usr_id = '{0}'", txtID.Text);
                        ds.Clear();
                        koneksi.Open();
                        perintah = new MySqlCommand(query, koneksi);
                        adapter = new MySqlDataAdapter(perintah);
